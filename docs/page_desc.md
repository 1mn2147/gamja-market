...
비밀번호는 12~128자이며 영문 대문자·소문자·숫자·공백이 아닌 특수문자를
각각 하나 이상 포함한다. 흔한 비밀번호 단어, 예측 가능한 연속 문자열, 같은
문자 4회 이상 반복, 이메일 아이디 또는 휴대전화 번호가 포함된 값은 사용할 수
없다. 모든 조건은 입력 중 체크(`✓`) 또는 엑스(`✕`)로 표시하고 색상에 의존하지
않는다. 화면낭독기에는 각 조건의 `충족` 또는 `필요` 상태를 함께 제공하며
서버에서도 동일하게 검증한다.
...
이동을 안내한다. 회원가입과 동일한 비밀번호 안전 정책을 서버에서 적용하며,
취약한 비밀번호 거부만으로 유효한 재설정 인증 코드를 소비하지 않는다.
...
| --- | --- | --- |
| 요청 | 요청 취소 | 수락 또는 거절 |
| 판매자수락 | 에스크로 결제 또는 기존 주문 계속 | 결제 상태·기한 확인, 결제 대기 |
| 결제완료 | 직거래 진행 | 직거래 진행 |
...
판매자 수락 상태에서는 결제 상태가 `APPROVED`일 때만 인도 완료 행동을 노출한다.
결제 전에는 결제 대기 안내와 수락 후 7일 기한을 표시하고, 이미 생성된 `READY`
주문이 있으면 구매자에게 새 주문 생성 대신 기존 결제 계속 링크를 제공한다.
경쟁 요청이 종료된 경우 내부 코드 대신 “다른 구매자의 요청이 먼저
수락되었습니다”라는 안전한 사용자 안내를 표시한다.
