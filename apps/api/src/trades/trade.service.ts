...
export class TradeService {
  ...
  async deliver(user: AuthenticatedUser, tradeId: string) {
    const trade = await prisma.$transaction(async (tx) => {
      const trade = await tx.trade.findFirst({ where: { id: tradeId, sellerId: user.id, status: TradeStatus.ACCEPTED } });
      if (!trade) throw new NotFoundException({ code: 'TRADE_NOT_FOUND' });
  ...
  async confirm(user: AuthenticatedUser, tradeId: string) {
    const trade = await prisma.$transaction(async (tx) => {
      const trade = await tx.trade.findFirst({ where: { id: tradeId, buyerId: user.id, status: TradeStatus.DELIVERED } });
      if (!trade) throw new NotFoundException({ code: 'TRADE_NOT_FOUND' });
      const updated = await tx.trade.update({ where: { id: tradeId }, data: { status: TradeStatus.CONFIRMED, confirmedAt: new Date() } });
  ...
  async cancel(user: AuthenticatedUser, tradeId: string, reason: string) {
    const trade = await prisma.$transaction(async (tx) => {
      ...
      if (!trade) throw new NotFoundException({ code: 'TRADE_NOT_FOUND' });
      if (trade.status === TradeStatus.ACCEPTED && trade.buyerId !== user.id) {
        throw new ForbiddenException({ code: 'ONLY_BUYER_CAN_CANCEL_ACCEPTED_TRADE' });
      }
      if (trade.createdAt.getTime() + 7 * DAY_MS < Date.now()) {
        throw new ConflictException({ code: 'CANCELLATION_WINDOW_EXPIRED' });
      }
